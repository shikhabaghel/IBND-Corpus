...
def convert_to_wav(source, destination, split_dur):
    command = 'ffmpeg -i ' + source + ' -ab 160k -ac 1 -ar 16000 -vn ' + destination
    ...
    with open(url_list, newline='\n') as csvfile:
        ...
        for row in csv_data:
            ...
            try:
                ...
                if not os.path.exists(opDir_path + '/audio/'+row['Debate_Name']):
                    os.makedirs(opDir_path + '/audio/'+row['Debate_Name']) 
                wav_fName = save_dir.replace('video','audio') +row['Debate_Name'] +'/'+fName.split('.')[0] + '.wav'
                print(wav_fName)
